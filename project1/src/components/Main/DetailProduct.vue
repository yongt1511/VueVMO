              ...
              tế.</span>
                            ...
                            <div class=" ">
                                <div class="btn-group btn-detail">
                                    <input class="select-quantity" type="number" id="quantity" name="quantity" value="1" v-model="offerProduct.quantity">
                                    <button type="button" class="btn btn-sm btn-outline-secondary addcartdetail"
                                            v-on:click="AddCart">Thêm vào giỏ hàng
                                    </button>
                                </div>
                            </div>
                        </div>
    ...
    export default {
        ...
        methods: {
                ...
                return new Promise((resolve, reject) => {
                    axios.get(`http://localhost:3000/product/${this.id}`)
                        .then(({data}) => {
                            ...
                            this.offerProduct.name = data.name;
                            this.offerProduct.price = data.price;
                            this.offerProduct.image = data.image;
                            this.offerProduct.product_id = data.id;
            ...
            AddCart: function () {
                ...
                return new Promise((resolve, reject) => {
                    axios.post(' http://localhost:3000/order',this.offerProduct)
                        .then(() => {
